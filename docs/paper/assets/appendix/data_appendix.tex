...
This report analyses laboratories 2 (Bern), 3 (Copenhagen), and 5 (Lyon): healthy wild-type sites with complementary montages, excluding laboratories 1 and 4 (pathology-specific or single-electrode cohorts).
...
\paragraph*{S8 Table.}
\label{S8_Table}
\apptabdesc{\textbf{Quality-screened cohort inventory.} Per-mouse recording runs, hours, and 4\,s epoch counts (MSSV annotation grid).}
\apptabular{\input{assets/tables/S8_cv4fold_cohort.tex}}
...
For substage physiology panels (Fig~\ref{fig3}, \nameref{S1_Fig}) and summary tables (\nameref{S3_Table}), log-power spectra are aggregated into interpretable bands on the same frequency grid:
...
\paragraph*{S9 Table.}
\label{S9_Table}
\apptabdesc{\textbf{Four-fold leave-mice-out assignments.} Top: held-out mice per site and fold.
