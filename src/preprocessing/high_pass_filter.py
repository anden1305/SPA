...
class HighPassFilter(BaseTransform):
    ...
    def validate_config(self, _: TransformsConfig):
        ...
        assert isinstance(self.config.params['low_cutoff'], (int, float)) and 0 < self.config.params['low_cutoff'] < self.sampling_rate / 2, "Low cutoff must be a positive number less than Nyquist frequency."
        assert isinstance(self.config.params['high_cutoff'], (int, float)) and 0 < self.config.params['high_cutoff'] < self.sampling_rate / 2, "High cutoff must be a positive number less than Nyquist frequency."
