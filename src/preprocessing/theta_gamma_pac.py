...
class ThetaGammaPAC(BaseTransform):
    """
    Theta–Gamma Phase–Amplitude Coupling (Tort Modulation Index) per window & channel.

    Purpose:
      - High in REM (stable theta modulating gamma amplitude).
      - Moderate / variable in Awake (task-dependent).
      - Low in NREM (weak coupling).
    Orthogonal to band-power features; measures cross-frequency coordination.
    ...
    def __call__(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(N):
            for c in range(C):
                ...
                # 1) Theta phase
                x_theta = self._fft_bandpass(xi, self.theta_lo, self.theta_hi, fs)
                z_theta = self._hilbert_analytic(x_theta)
                phase = np.angle(z_theta)  # [-pi, pi]

                # 2) Gamma amplitude envelope
                x_gamma = self._fft_bandpass(xi, self.gamma_lo, self.gamma_hi, fs)
                z_gamma = self._hilbert_analytic(x_gamma)
                amp = np.abs(z_gamma) + self.eps
    ...
    def get_short_name(self):
        return "ThetaGammaPAC"

    def __str__(self) -> str:
        return (
            "ThetaGammaPAC(window_size="
