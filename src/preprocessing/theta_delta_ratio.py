...
class ThetaDeltaRatio(BaseTransform):
    """Compute theta/delta power ratio per channel window.

    Input is (T, C) data and (T,) labels. Output contains a single feature per
    channel and window: theta_power / delta_power.
    """
    ...
    def __init__(self, config: TransformsConfig, window_size: int, stride: int, sampling_rate: int):
        ...
        self.fft = FFT(
            ...
            bands=[self._DELTA_TARGET, self._THETA_TARGET],
        )
    ...
    def __call__(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        ratio = theta_power - (delta_power) # theta_power / (delta_power + self.eps)
