...
class ThetaToBetaGammaRatio(BaseTransform):
    ...
    def __init__(self, config: TransformsConfig, window_size: int, stride: int, sampling_rate: int):
        ...
        self.fft = FFT(
            ...
            bands=[self._THETA_TARGET, self._BETA_TARGET, self._GAMMA_TARGET],
        )
    ...
    def __call__(self, x: np.ndarray, y: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        band_features = band_features.reshape(band_features.shape[0], 1, num_channels, 3)

        theta_power = band_features[..., self._THETA_INDEX]
        beta_power  = band_features[..., self._BETA_INDEX]
        gamma_power = band_features[..., self._GAMMA_INDEX]
        
        denom = beta_power + gamma_power
        ratio = theta_power / denom
    ...
    def get_short_name(self):
        return "ThetaToBetaGammaRatio"

    def __str__(self) -> str:
        return (
            "ThetaToBetaGammaRatio(window_size="
