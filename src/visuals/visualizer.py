    ...
    def __plot_pca_tripanel(self, train_details: TrainDetails, x: Tensor, y: Tensor, overwrite_path: str = None, pred_y: np.ndarray = None):
        ...
        for a, b in combinations(range(proj.shape[1]), 2):
            ...
            for ax, arr, t in zip(axes, arrays, titles):
                panel_names = state_names if t == "True" else None
                color_lut = scatter_layered(
                    ax,
                    proj[:, a],
                    proj[:, b],
                    arr,
                    label_names=panel_names,
                    palette=palette,
                    point_size=6,
                )
                ...
                if state_names is not None and len(state_names) > int(unique_labels.max()):
                    label_texts = [state_names[int(label)] for label in unique_labels]
                else:
                    label_texts = [f"State {int(label)}" for label in unique_labels]

                legend_elements = [
                    plt.Line2D(
                        [0], [0], marker="o", color="w",
                        markerfacecolor=color_lut[int(label)],
