...
def plot_fig2_dual(csv_path: Path, summary_path: Path, out: Path) -> None:
    ...
    fig, (ax_a, ax_b) = plt.subplots(
        1, 2, figsize=(6.6, 2.15), gridspec_kw={"width_ratios": [0.95, 1.15], "wspace": 0.38}
    )

    # --- Panel A: population mean (compact) ---
    panel_label(ax_a, "A", x=-0.22, y=1.12)
    means, mins, maxs = [], [], []
    ...
    ax_a.bar(x, means, color=colors, width=0.52, edgecolor="white", linewidth=0.6, zorder=3)
    for i, (m, lo, hi) in enumerate(zip(means, mins, maxs)):
        if np.isfinite(m):
            ax_a.errorbar(i, m, yerr=[[m - lo], [hi - m]], fmt="none", ecolor="#64748B", capsize=3, lw=1.0, zorder=4)
            ax_a.text(i, min(hi + 0.025, 0.74), f"{m:.2f}", ha="center", fontsize=7.5, fontweight="bold")
    if thesis is not None:
        ax_a.axhline(thesis, color=LADDER_COLORS["hmm_features"], linestyle="--", lw=1.0, alpha=0.85, zorder=1)
    ax_a.set_xticks(x)
    ax_a.set_xticklabels([LADDER_LABELS[k] for k in compare_keys], fontsize=7.5)
    ax_a.set_ylabel("Prior NMI", fontsize=8)
    ...
    if np.isfinite(delta):
        ax_a.text(0.97, 0.92, f"$\\Delta$=+{delta:.2f}", transform=ax_a.transAxes, ha="right", va="top",
                  fontsize=7.5, fontweight="bold", color=LADDER_COLORS["chmmgmvae_locked"])

    # --- Panel B: per-fold grouped (compact) ---
    ...
    for i, key in enumerate(compare_keys):
        ...
        for b, v in zip(bars, vals):
            if np.isfinite(v):
                ax_b.text(b.get_x() + b.get_width() / 2, v + 0.015, f"{v:.2f}", ha="center", fontsize=6)

    if thesis is not None:
        ax_b.axhline(thesis, color=LADDER_COLORS["hmm_features"], linestyle="--", lw=1.0, alpha=0.85)
    ax_b.set_xticks(x)
    ax_b.set_xticklabels([f"F{f}" for f in FOLD_ORDER], fontsize=7)
    ax_b.set_ylabel("Prior NMI", fontsize=8)
    ax_b.set_ylim(0, 0.72)
    ax_b.legend(frameon=False, loc="upper left", fontsize=6.5, handlelength=1.2, borderpad=0.2)
...
def plot_by_fold(csv_path: Path, out: Path, *, four_rung: bool = False) -> None:
    """S1 Fig — per-fold grouped bars (2 or 4 rungs when data available)."""
    apply_paper_style()
    by_fold = _load_joint_csv(csv_path)
    compare_keys = (
        ("hmmgmvae_locked", "cgmvae_locked", "chmmgmvae_locked")
        if four_rung
        else ("cgmvae_locked", "chmmgmvae_locked")
    )
    if four_rung:
        # Prepend if any fold has hmmgmvae data
        has_hmm = any(by_fold[f].get("hmmgmvae_locked") for f in FOLD_ORDER)
        if not has_hmm:
            compare_keys = ("cgmvae_locked", "chmmgmvae_locked")
    width = 0.34
    x = np.arange(len(FOLD_ORDER))
...
def main() -> int:
    ...
    parser.add_argument("--out", type=Path, default=REPO / "docs/paper/figures/archive/Fig2.pdf")
    parser.add_argument("--by-fold", action="store_true", help="S1 only (skip dual Fig 2)")
    parser.add_argument("--four-rung", action="store_true", help="Include HMMGMVAE when present in CSV")
    args = parser.parse_args()

    if args.by_fold:
        out = (
            args.out
            if args.out != REPO / "docs/paper/figures/archive/Fig2.pdf"
            else REPO / "docs/paper/figures/supplementary/S1_ladder_by_fold.pdf"
        )
        plot_by_fold(args.csv, out, four_rung=args.four_rung)
        import shutil
