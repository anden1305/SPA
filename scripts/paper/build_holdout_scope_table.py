...
def _metric_cell(metrics: dict, scope: str, fold: str, lab: str, model: str) -> str:
    key = (scope, fold, lab, model)
    if key not in metrics:
        return "---"
    m = metrics[key]
    return f"{m['nmi']:.2f}/{m['macro_accuracy']:.2f}"


def write_detailed_tex(path: Path, metrics: dict[tuple[str, str, str, str], dict[str, float]]) -> None:
    """Compact pivot: fold $\\times$ site rows, model columns with NMI/Acc per cell."""
    model_cols = [
        ("hmm_raw", "Raw"),
        ("cgmvae_locked", "cGM"),
        ("hmmgmvae_locked", "HGM"),
        ("chmmgmvae_locked", "cHMM"),
    ]
    site_short = {"lab_2": "Bern", "lab_3": "Cph", "lab_5": "Lyon"}

    def block(scope: str, title: str) -> list[str]:
        header = " & ".join(label for _, label in model_cols)
        out = [
            f"\\textbf{{{title}}}\\\\",
            "\\begin{tabular}{|c|l|cccc|}",
            "\\hline",
            f"\\textbf{{Fold}} & \\textbf{{Site}} & {header} \\\\",
            "\\thickhline",
        ]
        for fold in FOLDS:
            for i, lab in enumerate(LABS):
                fold_cell = fold if i == 0 else ""
                cells = " & ".join(_metric_cell(metrics, scope, fold, lab, model) for model, _ in model_cols)
                out.append(f"{fold_cell} & {site_short[lab]} & {cells} \\\\")
            if fold != FOLDS[-1]:
                out.append("\\hline")
    ...
    lines = [
        ...
        *block("joint", "Joint training (evaluate on held-out site mice only)"),
        "\\par\\medskip",
        *block("within_lab", "Within-site training"),
    ]
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_joint_fold_tex(path: Path, rows: list[dict[str, str]], hmm_metrics: dict) -> None:
    ...
    for model in header_models:
        ...
        if model == "hmm_raw" and n < len(FOLDS):
            mean_cells.append("---")
        elif n == 0:
            mean_cells.append("---")
        else:
...
def main() -> int:
    ...
    for r in collect_hmm_rows(manifest):
        if r.scope != "joint" or r.nmi is None:
            continue
        if r.status == "ok" and r.macro_accuracy is not None:
            hmm_pooled[("joint", r.fold, "all", "hmm_raw")] = {
                "nmi": r.nmi,
                "macro_accuracy": r.macro_accuracy,
            }
        elif r.status == "partial_log":
            hmm_pooled[("joint", r.fold, "all", "hmm_raw")] = {
                "nmi": r.nmi,
